import { pgTable, serial, text, integer, doublePrecision, timestamp, varchar, unique, foreignKey, index } from 'drizzle-orm/pg-core';
...
export const pageStat = pgTable('page_stat', {
  id: serial('id').primaryKey(),
  ...
  deviceFk: foreignKey({ columns: [t.deviceId], foreignColumns: [device.id] }).onDelete('cascade'),
}));
...
export const bookDevice = pgTable('book_device', {
  id: serial('id').primaryKey(),
  bookMd5: varchar('book_md5', { length: 32 }).notNull(),
  deviceId: varchar('device_id', { length: 255 }).notNull(),
  lastOpen: integer('last_open').default(0),
...
export const annotation = pgTable('annotation', {
  ...
  bookFk: foreignKey({ columns: [t.bookMd5], foreignColumns: [book.md5] }).onDelete('cascade'),
  deviceFk: foreignKey({ columns: [t.deviceId], foreignColumns: [device.id] }).onDelete('cascade'),
  deletedIdx: index('idx_annotation_deleted_at').on(t.deletedAt).where(sql`deleted_at IS NOT NULL`),
}));
